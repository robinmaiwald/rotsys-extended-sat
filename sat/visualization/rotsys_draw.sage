...
def find_planarization(rs,DEBUG=False):
    ...
    var_order_ = {(a,b,i,v):vpool.id() for a,b in combinations(N,2) for i in range(len(vertices_along_edge[a,b])) for v in vertices_along_edge[a,b]}
    var_order2_ = {(a,b,i,u,v):vpool.id() for a,b in combinations(N,2) for i in range(1,len(vertices_along_edge[a,b])) for u,v in permutations(vertices_along_edge[a,b],2)}
    var_edge_ = {(u,v):vpool.id() for u,v in permutations(all_vertices,2)}
    def var_order(*L): return var_order_[L]
    def var_order2(*L): return var_order2_[L]
    def var_edge (*L): return var_edge_[L]

    # variables for planarity
    var_is_smaller_ = {(i,u,v):vpool.id() for i in range(3) for u,v in permutations(all_vertices,2)} # u < v in i-th linear order 
    var_3rd_is_biggest_ = {(i,u,v,w):vpool.id() for i in range(3) for u,v,w in permutations(all_vertices,3) if u<v} # u < w and v < w in i-th linear order 
    def var_is_smaller(*L): return var_is_smaller_[L]
    def var_3rd_is_biggest(*L): return var_3rd_is_biggest_[L]
